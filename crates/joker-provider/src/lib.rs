...
// ── ReconnectingModel (DeepSeek-Reasonix streamWithReconnect) ───────────────

/// Wraps an inner `Model` and retries the stream on connection failure,
/// but only when **no output has been emitted yet** (zero text deltas, zero
/// tool calls). This mirrors DeepSeek-Reasonix's `streamWithReconnect` which
/// re-streams the request up to 3 times only on zero-output failures.
...
impl ReconnectingModel {
    /// Create a new reconnecting wrapper with default settings (3 retries, 1s base delay).
    #[must_use]
...
/// Detects whether the stream emitted any events before failing.
/// If not, it retries with exponential backoff.
...
/// Simple receiver-based stream adapter.
